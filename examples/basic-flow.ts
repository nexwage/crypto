...
(async () => {
  ...
  const { masterKey, wrapped: wrappedMasterKey } =
    await generateMasterKeyWithPassword(password, {}, aad);
  ...
  console.log(
    "test value",
    sodium.memcmp(recoveryKey, decRecoveryKeyBuffArray)
  );

  const wrappedMasterKeyByRecovery = await wrapMasterKeyWithRecoveryKey(
    masterKey,
    recoveryKey,
    aad
  );
  ...
  const wrappedMasterKeyNew = await generateMasterKeyWithPassword(
    newPassword,
    {},
    aad
  );
  const wrappedMasterKeyByRecoveryNew = await wrapMasterKeyWithRecoveryKey(
    masterKey3,
    recoveryKey,
    aad
  );
